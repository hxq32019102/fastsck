...
def get_dir_by_name_parent_id(db: Session, dirname: str, parent_id: int):
    return db.query(models.DirInfo).filter(models.DirInfo.parent_id == parent_id,
                                           models.DirInfo.dir_name == dirname).first()
...
def get_file_by_dirid_filename(db: Session, dirid: int, filename: str):
    return db.query(models.FileInfo).filter(models.FileInfo.dir_id == dirid,
                                            models.FileInfo.file_name == filename).first()
...
def create_file(db: Session, dir_id: int, file_name: str, file_size: int, uid: str):
    db_file = models.FileInfo(dir_id=dir_id, file_name=file_name, file_size=file_size, file_upload_time=datetime.now(),
                              uid=uid)
    db.add(db_file)
    db.commit()
    db.refresh(db_file)
